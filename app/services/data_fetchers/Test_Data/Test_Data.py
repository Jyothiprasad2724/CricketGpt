...
def save_combined_test_data():
    ...
    comp_code = 'T'  # Test format
    combined_df = []

    for country in country_codes:
        print(f"Fetching data for {country} - TEST...")
        df = fetch_players(country_code=country, comp=comp_code)
        if not df.empty:
            combined_df.append(df)
        else:
            print(f"No data found for {country} - TEST")

    if combined_df:
        final_df = pd.concat(combined_df, ignore_index=True)
        output_path = r"C:\Users\jyothi.p.kommuru\Downloads\CricketGpt Project\CricketGpt\app\services\data_fetchers\Test_Data\all_teams_test_data.csv"
